...
class PickPlace:
    ...
    def attach_object(self, name):
        ...
        attached.touch_links            = ['link_3', 'link_4']
        scene                           = PlanningScene()
        scene.is_diff                   = True
    ...
    def detach_object(self, name):
        """
        Detach the object from the gripper in MoveIt's scene.
        Does NOT re-add it to the world — call add_box separately.
        """
        attached                  = AttachedCollisionObject()
        attached.link_name        = 'tcp_link'
        attached.object.id        = name
    ...
    def run(self, pick_x, pick_y, pick_z,
        ...
        # Pre-flight safety corrections
        place_z          = self._safe_z(place_z)
        place_x, place_y = self._avoid_singularity(place_x, place_y)

        # Transit midpoint — diagonal, never (0,0) which is base singularity
        mid_x = (pick_x + place_x) / 2.0
        mid_y = (pick_y + place_y) / 2.0
        mid_x, mid_y = self._avoid_singularity(mid_x, mid_y)
        ...
        log.info(f'    Transit: ({mid_x:.3f}, {mid_y:.3f}, {TRANSIT_Z:.3f})')
        log.info('=' * 50)
        ...
        self._move(pick_x, pick_y, pick_z + 0.08, label='pick-hover')
        ...
        # ── 8. Swing to midpoint ───────────────────────────────────────────
        log.info('📌 Step 8 — Transit swing')
        self._move(mid_x, mid_y, TRANSIT_Z, label='transit')

        # ── 9. Fly to above place ──────────────────────────────────────────
        log.info('📌 Step 9 — Approach place high')
        self._move(place_x, place_y, TRANSIT_Z, label='place-high')

        # ── 10. Intermediate descent ───────────────────────────────────────
        log.info('📌 Step 10 — Intermediate descent')
        self._move(place_x, place_y, place_z + 0.06, label='place-mid')

        # ── 11. Final descent ──────────────────────────────────────────────
        log.info('📌 Step 11 — Final descent')
        self._move(place_x, place_y, place_z, label='place-contact')

        # ══════════════════════════════════════════════════════════════════
        # RELEASE SEQUENCE — must fully complete before any motion resumes
        # ══════════════════════════════════════════════════════════════════

        # 12a. Open gripper wide (ensures fingers clear the object)
        log.info('📌 Step 12a — Open gripper WIDE')
        self.gripper.grip_box(grip_width * 1.5)   # open wider than the box
        time.sleep(0.8)                            # wait for fingers to open fully

        # 12b. Open again to maximum just to be sure
        log.info('📌 Step 12b — Full gripper open')
        self.gripper.open()
        time.sleep(0.8)

        # 13a. Detach from MoveIt scene
        log.info('📌 Step 13a — Detach from MoveIt scene')
        self.detach_object(object_name)
        time.sleep(0.5)

        # 13b. Wait for MoveIt to confirm detach before moving
        log.info('📌 Step 13b — Confirming detach...')
        self.confirm_detach(object_name, timeout=4.0)

        # 13c. Re-register box at place position in world scene
        log.info('📌 Step 13c — Register box at place location')
        self.add_box(object_name,
                     place_x, place_y, place_z,
                     box_size_x, box_size_y, box_size_z)
        self._spin(duration=0.5)   # let scene update propagate

        # 13d. Save scene state for next run
        self.save_scene_state(object_name,
                              place_x, place_y, place_z,
                              box_size_x, box_size_y, box_size_z)

        # ══════════════════════════════════════════════════════════════════
        # RETREAT — move straight UP first, then away from place entirely.
        # Never swing back through the pick column — that's what caused
        # the arm to re-grip the object.
        # ══════════════════════════════════════════════════════════════════

        # 14a. Lift straight up from place (same XY, just raise Z)
        log.info('📌 Step 14a — Lift straight up from place')
        self._move(place_x, place_y, TRANSIT_Z, label='retreat-lift', delay=0.6)

        # 14b. Park above the pick column — already proven reachable.
        # Gripper is fully open so no risk of re-gripping.
        log.info('📌 Step 14b — Park above pick column (known-reachable XY)')
        self._move(pick_x, pick_y, TRANSIT_Z, label='park', delay=0.6)

        log.info('=' * 50)
        log.info('🎉  Pick & Place complete!')
        log.info(f'📦  Box placed at ({place_x:.3f}, {place_y:.3f}, {place_z:.3f})')
        log.info(f'🏠  Arm parked above pick ({pick_x:.3f}, {pick_y:.3f}, {TRANSIT_Z:.3f})')
        log.info('=' * 50)
